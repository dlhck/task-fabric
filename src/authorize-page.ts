export interface AuthorizePageParams {
  ...
  scope: string;
  resource: string;
  /** When true, a valid consent cookie was already presented — skip the API key field. */
  trusted?: boolean;
}
...
export function renderAuthorizePage(params: AuthorizePageParams): string {
  ...
  const apiKeyField = params.trusted
    ? `<p class="trusted">This browser is already authorized. Approve to continue.</p>`
...
<head>
  ...
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .trusted { color: #8b8b8b; font-size: 0.85rem; margin-bottom: 1.25rem; padding: 0.75rem; background: #0d1a2e; border: 1px solid #1e3a5f; border-radius: 6px; }
...
<body>
  <div class="card">
    ...
    <form method="POST" action="/authorize/decide">${hiddenFields}
      ${apiKeyField}
      <div class="actions">
